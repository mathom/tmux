...
const struct cmd_entry cmd_choose_tree_entry = {
	"choose-tree", NULL,
	"S:W:b:c:t:", 0, 1,
	"[-S format] [-W format] [-b session template] [-c window template] " \
...
cmd_choose_tree_exec(struct cmd *self, struct cmd_ctx *ctx)
{
	...
	/* Pad the win template so it looks like part of a tree. */
	xasprintf(&final_win_template, "   %s%s> %s", tty_acs_get(tty, 't'),
		tty_acs_get(tty, 'q'),	win_template);
